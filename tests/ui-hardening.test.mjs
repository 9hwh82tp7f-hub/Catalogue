...
assert.match(sw, /CACHE_NAME = CACHE_PREFIX \+ 'v13\.0-[^']+'/, 'Le cache local doit être invalidé après la passe panel/index.');

assert.match(html, /function disableDeadLinks\(root=document\)/, 'Les liens sociaux/contact sans destination doivent être neutralisés.');
assert.match(html, /function setOptionalExternalLink\(id, value/,
  'Les liens communautaires doivent avoir un état explicite quand ils ne sont pas configurés.');
assert.doesNotMatch(html, /communityBtn[^\n]*safeUrl\(CONFIG\.telegramChannelUrl, 'href'\) \|\| '#'/,
  'Le lien communautaire ne doit pas retomber sur href="#".');
assert.match(html, /faqListToggle\.dataset\.faqBound !== '1'/,
  'Le bouton FAQ ne doit pas empiler des listeners lors des reconstructions.');
...
console.log('UI hardening/regression tests: OK');
