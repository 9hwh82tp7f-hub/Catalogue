...
assert.match(html, /function openCart\(\)/, 'Le bouton panier mobile doit disposer d’une fonction openCart réelle.');
assert.match(html, /mobileCartOpen[^\n]*|mobileCartOpen.*openCart/s, 'Le bouton panier mobile doit appeler openCart.');
...
assert.match(html, /\.mobile-cart-bar\{[\s\S]*?bottom:calc\(var\(--nav-height\)[\s\S]*?z-index:105/, 'La barre panier mobile doit être au-dessus de la navigation basse.');
assert.match(html, /@media \(max-width:720px\)\{[\s\S]*?\.cart-fab\{ display:none; \}/, 'Le FAB panier doit être masqué sur mobile pour éviter le doublon avec la barre panier.');
...
assert.match(sw, /CACHE_NAME = CACHE_PREFIX \+ 'v10\.3-20260816-final'/, 'Le cache local doit être invalidé après la passe UI.');
