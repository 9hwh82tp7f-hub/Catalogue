...
(function () {
  ...
  function readSettings() {
    try {
      return JSON.parse(localStorage.getItem("visualSettings") || "{}");
    } catch (_) {
      return {};
    }
  }
  ...
  function intensity() {
    ...
    return clamp(Number(s.threeDIntensity ?? s["3dIntensity"] ?? 65) / 65, 0, 1.5);
  }

  function threeDEnabled() {
    const s = readSettings();
    return s.enable3D !== false && s.threeD !== false && intensity() > 0;
  }

  function performanceTier() {
    const s = readSettings();
    let tier = s.performanceMode || s.quality || "auto";
    if (tier === "auto") {
      const cores = navigator.hardwareConcurrency || 4;
      const memory = navigator.deviceMemory || 4;
      tier = cores <= 4 || memory <= 4 ? "low" : "balanced";
    }
    return ["low", "balanced", "ultra"].includes(tier) ? tier : "balanced";
  }

  function applyRuntimeState() {
    const enabled = threeDEnabled() && !reduceMotion.matches && !coarsePointer.matches;
      ...
      el.querySelectorAll("button,a,[role='button']").forEach((button) => {
        ...
        if (!button.dataset.magneticBound) {
          ...
          button.addEventListener("pointermove", (event) => {
            if (reduceMotion.matches || coarsePointer.matches || !body.classList.contains("ui-3d-enabled")) return;
  ...
  function setupAmbient() {
    if (!threeDEnabled() || reduceMotion.matches || coarsePointer.matches || performanceTier() === "low") return;

    if (!document.querySelector(".v4-cursor-light")) {
  ...
  function setupShowroom() {
    ...
    if (!stage || reduceMotion.matches || coarsePointer.matches || !threeDEnabled()) return;
